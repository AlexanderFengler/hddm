...
def wiener_like_nn_ddm_pdf(np.ndarray[float, ndim = 1] x,
    ...
    # Call to network:
    if p_outlier == 0: # ddm_model
        log_p = np.core.umath.maximum(network.predict_on_batch(data), ll_min)
    else: # ddm_model
        log_p = np.log(np.exp(np.core.umath.maximum(network.predict_on_batch(data), ll_min)) * (1.0 - p_outlier) + (w_outlier * p_outlier))
        #log_p = np.sum(np.log(np.add(np.multiply(np.exp(np.core.umath.maximum(ddm_model.predict_on_batch(data), ll_min)), 
        #                    (1.0 - p_outlier)), 
        #              (w_outlier * p_outlier))))
    if logp == 0:
...
def wiener_like_rlddm(np.ndarray[double, ndim = 1] x,
    ...
    for j in range(unique.shape[0]):
        ...
        for i in range(1, s_size):
            ...
            sum_logp += log(p * (1 - p_outlier) + wp_outlier)
            #sum_logp += log(p)
...
def wiener_like_rl(np.ndarray[long, ndim=1] response,
    ...
    for j in range(unique.shape[0]):
        ...
        for i in range(1, s_size):
            ...
            # Unnecessary since drift should pretty much never be 0 ...
            # if drift == 0:
            #     p = 0.5
            # else:
